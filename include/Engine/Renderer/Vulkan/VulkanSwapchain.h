...
public:
  VulkanSwapchain(VkInstance instance) { this->instance = instance; }
  ...
  void create(uint32_t width, uint32_t height, bool vsync = false) {
    ...
    uint32_t desiredSwapchainImagesCount =
    ...
    for (uint32_t i = 0; i < imageCount; i++) {
      ...
      VkResult result = vkCreateImageView(device, &colorAttachmentView, nullptr,
                                          &buffers[i].view);
  ...
  VkFormat getImageFormat() { return colorFormat; }

  std::vector<SwapChainBuffer> *getSwapChainBuffers() { return &buffers; }
