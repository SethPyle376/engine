...
VulkanBuffer* VulkanMeshResource::createBuffer() {
    ...
    VulkanBuffer stagingBuffer(device, size, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VMA_MEMORY_USAGE_CPU_ONLY);
    stagingBuffer.update(reinterpret_cast<const void*>(vertices.data()), size, 0);

    VulkanBuffer* gpuBuffer = new VulkanBuffer(device, size, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VMA_MEMORY_USAGE_GPU_ONLY);
